...
export NODE_PORT=$(kubectl get --namespace {{ $context.Release.Namespace }} -o jsonpath="{.spec.ports[0].nodePort}" services {{ include "bedag-lib.fullname" $context }})
...
export SERVICE_IP=$(kubectl get svc --namespace {{ $context.Release.Namespace }} {{ include "bedag-lib.fullname" . }} --template "{{"{{ range (index .status.loadBalancer.ingress 0) }}{{.}}{{ end }}"}}")
echo http://$SERVICE_IP:{{ .service.port }}{{ $path }}
        {{- else if contains "ClusterIP" .service.type }}
export POD_NAME=$(kubectl get pods --namespace {{ $context.Release.Namespace }} -l "app.kubernetes.io/name={{ include "lib.utils.name" $context }},app.kubernetes.io/instance={{ $context.Release.Name }}" -o jsonpath="{.items[0].metadata.name}")
