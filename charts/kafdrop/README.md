...
| autoscaling.maxReplicas | int | `2` | maxReplicas is the upper limit for the number of replicas to which the autoscaler can scale up. It cannot be less that minReplicas. |
...
| deployment.image.pullPolicy | string | `"IfNotPresent"` | Configure Docker Pull Policy. Will be overwritten if set by global variable. |
...
| proxy.httpProxy.port | string | `""` | (int) Configure HTTP Proxy Port |
